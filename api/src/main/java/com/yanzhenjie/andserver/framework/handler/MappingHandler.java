...
public abstract class MappingHandler implements MethodHandler {
    ...
    protected Map<String, String> getPathVariable(@NonNull String httpPath) {
        ...
        List<Path.Rule> pathList = mMapping.getPath().getRuleList();
        for (Path.Rule path : pathList) {
            ...
            if (pathStr.equals(httpPath)) return Collections.emptyMap();

            boolean matches = true;
            boolean isBlurred = false;
            for (int i = 0; i < segments.size(); i++) {
                Path.Segment segment = segments.get(i);
                isBlurred = segment.isBlurred();
                if (!segment.equals(httpSegments.get(i)) && !isBlurred) {
                    matches = false;
                    break;
                }
            }
